...
#else
constexpr pin_size_t HEARTBEAT_LED_PIN = 0;
constexpr pin_size_t SENSOR_LED_PIN = 0;
constexpr pin_size_t NETWORK_LED_PIN = 0;
constexpr pin_size_t POWER_LED_PIN = 0;
...
constexpr bool UI_HAS_USER_BUTTON = true;
#else
constexpr pin_size_t USER_BUTTON_PIN = 0;
...
void updateNetworkLed(unsigned long nowMs) {
  ...
  if (backlogStorage.count() > 0 || recentUploadFailure) {
...
void updatePowerLed(unsigned long nowMs) {
  ProbeReading powerSnapshot;
  copyLatestPowerSnapshotToReading(powerSnapshot);
