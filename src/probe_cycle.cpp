...
bool performProbeAndUpload(const char *reason) {
  ProbeReading r;
  const bool waterReadSucceeded = probeNow(r);
  if (!waterReadSucceeded) {
    r = ProbeReading();
    r.timestampUtc = nowUtcString();
    r.modbusId = detectedSensorId;
    r.sensorIdentity = detectedIdentity;
    refreshPowerForReading(r);
    readWeatherForReading(r);
    if (waterUnavailableObservations != UINT32_MAX) {
      ++waterUnavailableObservations;
    }
  }

  if (r.timestampUtc.length() == 0) {
    Serial.println("Site observation skipped: valid UTC timestamp unavailable");
    Serial.println("---");
    return false;
  }
  ...
  if (waterReadSucceeded) {
    ...
    Serial.println(
        "Water measurement unavailable; uploading partial site observation");
  }
