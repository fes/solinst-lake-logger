...
|---|---|---|
| UART TX | PB11 | Inkplate receive |
| UART RX | PB10 | Inkplate transmit |
| GND | GND | Common reference |

Both boards use 3.3 V signaling. Cross TX and RX, connect ground, and never
apply 5 V to either UART input. Final GIGA UART pins will be assigned when the
Inkplate is integrated into the field harness. The USB serial port accepts the
same protocol for bench setup without occupying the device UART.
...
The GIGA sender and production wiring are intentionally deferred until the
physical Inkplate is available for bench validation.
