...
| Modbus echo/noise | CRC, malformed-frame, offset-scanner, and bounded diagnostic-history unit tests | Introduce controlled echo/noise on the isolated bench bus | Valid replies are found; corrupt replies never become readings; terminal failures appear in chronological order in `/status`; recovery follows noise removal |
