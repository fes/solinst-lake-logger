// RS-485 diagnostic for Uno R4 WiFi with RS232/RS485 Shield V1 and a
// DFRobot SEN0657.
//
// Shield setup: RS485, UART, Auto direction control.
...
//   - Set this sketch to MODE_MASTER and the Opta sketch to MODE_MONITOR, or
//   - Set this sketch to MODE_MONITOR and the Opta sketch to MODE_MASTER.
//
// All nodes must share RS-485 A, B, and common/reference.

constexpr uint8_t MODE_MASTER = 1;
constexpr uint8_t MODE_MONITOR = 2;
constexpr uint8_t DIAGNOSTIC_MODE = MODE_MONITOR;
...
constexpr size_t MONITOR_PROFILE_INDEX = 1;
...
void configureRs485(const WeatherStationProfile &profile) {
  Serial1.begin(profile.baud, SERIAL_8N1);
  delay(20);
...
void sendWindSpeedRequest(const WeatherStationProfile &profile) {
  ...
  while (millis() - startMs < RESPONSE_TIMEOUT_MS) {
    while (Serial1.available() > 0 && responseLength < sizeof(response)) {
...
void setup() {
  ...
  Serial.println();
  Serial.println("Uno R4 SEN0657 RS-485 diagnostic");
  Serial.print("Mode: ");
